...
class Agent(object) :
    ...
    def __init__(self, model, policy = 'greedy', epsilon = 0.9, epsilon_decay = 0.001) :
        self.reset_guessed()
        if policy not in ['stochastic', 'greedy', 'epsilon_greedy'] :
            raise ValueError('Policy can only be stochastic or greedy')
        self._policy = 'epsilon_greedy'
    ...
    def select_action(self,state) :            
        ...
        if self._policy == 'epsilon_greedy':
            if np.random.rand() <= self.epsilon:  # Exploration: choose a random action
                # Make sure the random action is not in the guessed list
                guess = np.random.choice([l for l in letters if l not in self.guessed])
